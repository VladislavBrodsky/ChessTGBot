...
export default function LessonViewer({ steps, onComplete }: LessonViewerProps) {
 ...
 {(currentStep.type === 'text' || (currentStep.type === 'interactive_board' && !hasInteractiveSolution)) && (
   <div className={`w-full mx-auto flex flex-col ${currentStep.fen ? 'md:flex-row items-center' : ''} gap-6 md:gap-8`}>
      <div className="flex-1 p-8 md:p-10 rounded-3xl border border-brand-border-opacity-10 bg-brand-surface shadow-premium flex flex-col justify-center min-h-[300px] relative overflow-hidden group">
        ...
        {!stepComplete && (
          <button
            ...
            className="mt-8 w-full py-4 bg-emerald-500 hover:bg-emerald-600 text-white font-black uppercase tracking-widest rounded-xl hover:scale-[1.02] active:scale-95 transition-all shadow-[0_4px_20px_rgba(16,185,129,0.3)] hover:shadow-[0_4px_30px_rgba(16,185,129,0.5)] relative z-10 flex items-center justify-center gap-2 overflow-hidden border border-emerald-400/20"
          >
     ...
     {currentStep.fen && (
       <div className="flex-1 rounded-3xl overflow-hidden border border-brand-border-opacity-10 bg-brand-void p-2 md:p-3 shadow-[0_12px_40px_rgba(0,0,0,0.15)] flex items-center justify-center max-w-sm mx-auto md:max-w-md w-full relative">

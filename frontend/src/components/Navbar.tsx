...
export default function Navbar() {
    ...
    return (
        <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center bg-black/80 backdrop-blur-2xl border border-white/5 px-2 py-2 rounded-2xl shadow-premium min-w-[340px] justify-center">
            {/* Subtle glow overlay */}
            ...
            <ul className="flex items-center relative z-10 w-full justify-around space-x-1">
                {NAV_ITEMS.map((item) => {
                    ...
                    return (
                        <li key={item.href} className="flex-1 max-w-[68px] min-w-[60px]">
                            <Link href={item.href}>

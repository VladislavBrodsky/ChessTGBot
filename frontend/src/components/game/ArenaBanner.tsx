      ...
      {isLive && arena.standings.length > 0 && (
        <div className="mt-4 flex flex-col gap-1.5">
          {arena.standings.slice(0, 3).map((row) => (
            <div key={row.user_id} className={`flex items-center justify-between text-[11px] font-bold p-2.5 rounded-xl ${row.user_id === arena.me?.user_id ? 'bg-emerald-500/15 border border-emerald-500/30 text-emerald-400' : 'bg-brand-surface/50 border border-brand-primary/5 text-brand-primary/70'}`}>

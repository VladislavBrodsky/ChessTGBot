...
- **The platform's only money entry** is the USDT deposit transfer to the master wallet with a `ref_` comment. The in-app swap (STON.fi), Transak, and any self-custodial bridge flow deliver to the USER's wallet; keep it that way — don't add credit paths without dedup keys or credit from third-party status.
